...
//隨機輸入10商品(品名、價格、商家)，利用鏈結串列結構存資料，利用排序方法處理鏈結串列結構資料，按照價格由小到大排序並輸出結果。
	...
	int score;
	char st[30];
	status *link;                     // status 結構 : NAME - score - ST -   LINK(指向一個status) 
};
...
void insert(){
	...
	while(current != NULL && current->score < prt->score){   //當前score 小於輸入 score 
...
void show()
{
	...
	while (current != NULL) { //當current 不為空 
		cout << setw(15) <<left << current->name << setw(6) <<current->score  ; // 輸出 
